...
begin
  ...
  while listeNoeudsPossibles.Count > 0 do begin  // Tant que la liste des noeuds possibles n'est pas vide
    ...
    for var unVoisin in listeNoeudsVoisins.Keys do begin  // Parcours des noeuds voisins
      ...
      if not(listeNoeudsPossibles.ContainsKey(unVoisin)) then // Si le voisin n'est pas déjà dans la liste des noeuds possibles, on l'y rajoute
...
begin
  var iNoeud := noeudArrivee; // on part du noeud d'arrivée
...
begin
  if liste.Count > 0 then begin
    var tableau := liste.ToArray;   // Astuce pour récupérer le premier élément d'un TDictionary (pas de méthode first sur le TDictionary)
...
begin
  ...
  for var x := -1 to 1 do begin
    for var y := -1 to 1 do begin
      ...
      if not(AutoriserDeplacementDiagonal) then begin // si les déplacements en diagonal ne sont pas autorisés
