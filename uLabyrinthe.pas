...
type
  ...
  public
    ...
    procedure genererLabyrinthe(parfait : boolean = false);
  end;
...
procedure TLabyrinthe.genererLabyrinthe(parfait : boolean = false);
begin
  randomize;
  setlength(matrice, tailleX, tailleY);
  var nb := 0;
  for var i := 0 to tailleX-1 do   // Initialisation de la grille
  ...
  if not(parfait) then ouvrirLabyrinthe; // Si on souhaite un labyrinthe complexe (plusieurs chemins possibles, salles)
end;
